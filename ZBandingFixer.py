...
#Revision history: v1.0 (10/3/17), v1.01 (10/4/17)
...
import numpy as np
import pylab as p			#if not displaying the plot (by keeping ply.show() commented), commenting this will make the script load much faster.
...
A = .058			# Amplitude of deviation. Find by trial and error.
...
phase = .8			# Height above bed where deviation is 0 with positive derivative. May be difficult to measure. Trial and error likely needed to find it.
...
##This is a Fourier Series with each term on a separate line for readability. The first term by itself is a sine function. Add more terms by copying/ pasting and changing the coefficient of (2/pitch) to another integer with each new term, and adjusting the coefficient for A to play with the shape. Uncomment plt.show() below to see the shape.
def modz(zval):
	mz = 1 * (	\
	 (1.00 * A * np.sin(1 * (2/pitch)* np.pi * (zval - phase) ) + center)	\
	+(0.1 * A * np.sin(2 * (2/pitch)* np.pi * (zval - phase) ) + center)	\
	#+(0.07 * A * np.sin(3 * (2/pitch)* np.pi * (zval - phase) ) + center)	\
	#+(0.02 * A * np.sin(5 * (2/pitch)* np.pi * (zval - phase) ) + center)	\
	)
	return mz
...
plt.axis([-2, .5, 2*pitch, 0])		#Define axis ranges.
p.fill(y, x, facecolor='blue', alpha=0.5)			#Fill area to right of curve to show what side represets filled plastic
plt.title('Shape of part before correction:')
