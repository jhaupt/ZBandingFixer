...
def modz(zval):
	mz = -1 * (A * np.sin( (2*(1/pitch))* np.pi * (zval - phase) ) + center)		#MEAT: Z movement modification function.
	return mz


##Display shape of deviation over one period at the step resolution for reference. Useful when defining something other than a sine wave in modz. Uncomment plt.show() to see.
x = arange(0,pitch,stepsz)			#This is only used to display function.
y = modz(x)							#Send the array into modz
plt.plot(y,x)						#Set up the plot.
plt.axis([-.5, .5, 0, pitch])		#Define axis ranges.
